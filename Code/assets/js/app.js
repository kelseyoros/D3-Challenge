...
function xScale(healthData, chosenXAxis) {
  ...
  var xLinearScale = d3
    .scaleLinear()
    .domain([
      parseFloat(d3.min(healthData, (d) => d[chosenXAxis])) * 0.8,
      parseFloat(d3.max(healthData, (d) => d[chosenXAxis])) * 1.2,
    ])
    .range([0, width]);
...
function yScale(povertyData, chosenYAxis) {
  ...
  var yLinearScale = d3
    .scaleLinear()
    .domain([
      parseFloat(d3.min(povertyData, (d) => d[chosenYAxis])) * 0.8,
      parseFloat(d3.max(povertyData, (d) => d[chosenYAxis])) * 1.2,
    ])
    .range([height, 0]);
...
var data = d3
  ...
  .then(function (data, err) {
    ...
    //append circles
    var gGroup = chartGroup.selectAll("g circles").data(data).enter();
    ...
    xLabelsGroup.selectAll("text").on("click", function () {
      ...
      if (value !== chosenXAxis) {
        ...
        var xMin = d3.min(theData, function (d) {
          return parseFloat(d[chosenXAxis]) * 0.9;
        });

        // .max will grab the largest datum from the selected column.
        var xMax = d3.max(theData, function (d) {
          return parseFloat(d[chosenXAxis]) * 1.1;
        });

        var myxScale = d3.scaleLinear().domain([xMin, xMax]).range([0, width]);
        ...
        d3.selectAll("circle").each(function () {
          ...
          d3.select(this)
            .transition()
            .attr("cx", (d) => myxScale(d[chosenXAxis]))
            .duration(300);
        });
